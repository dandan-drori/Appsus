...
function query() {
  return storageService.query(BOOKS_KEY).then((books) => {
    if (!books.length) {
      return storageService.postMany(BOOKS_KEY, gBooksData);
    } else return books;
  });
}
...
function addReview(bookId, review) {
  return storageService.get(BOOKS_KEY, bookId).then((book) => {
    // console.log(book.reviews, review);
    console.log(book);
...
function removeReview(reviewId, bookId) {
  return storageService.get(BOOKS_KEY, bookId).then((book) => {
    const idx = book.reviews.findIndex((review) => {
...
function getGoogleBooks(searchWord) {
  return axios
    .get(
      `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchWord}`
    )
    .then((res) => {
